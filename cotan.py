...
def scale_tensor(A, B):
    """
    Scale tensor A to have a similar scale to tensor B.
    
    Args:
    A (torch.Tensor): Tensor to be scaled, shape (1, N, 3)
    B (torch.Tensor): Reference tensor, shape (1, N, 3)
    
    Returns:
    torch.Tensor: Scaled version of A
    """
    ...
    scale_factor_std = B_flat.std() / A_flat.std()
    A_scaled_std = A * scale_factor_std
    
    # Method 2: Using mean absolute value
    scale_factor_mean = B_flat.abs().mean() / A_flat.abs().mean()
    A_scaled_mean = A * scale_factor_mean
    
    return A_scaled_std, A_scaled_mean
...
class CoTAN(nn.Module):
    ...
    def forward(self, x,f, T, age, vol):
        ...
        normals = compute_vert_normal(x,f.long())
        
        # ------ integration ------
        for n in range(N):
            # sample velocity for each vertex
            v = self.interpolate(x, vel_field)  # (M*R,|V|,3)
            _, A_scaled_mean = scale_tensor(normals , v[0,...])
            v =torch.cat([v,A_scaled_mean ], dim = 0)
